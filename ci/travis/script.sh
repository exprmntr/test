...
{
    brew install wget
    wget https://developer.nvidia.com/compute/cuda/8.0/Prod2/local_installers/cuda_8.0.61_mac-dmg -o cuda_8.0.61_mac.dmg
...
if [ "${CB_BUILD_AGENT}" == 'python2-darwin-x86_64-release' ]; then
     install_cuda_darwin;
     cd catboost/python-package;
     python2.7 ./mk_wheel.py -T -j 1 -DCUDA_ROOT=/usr/local/cuda;
fi
...
if [ "${CB_BUILD_AGENT}" == 'python35-darwin-x86_64-release' ]; then
     ...
     pyenv install 3.5.2;
     cd catboost/python-package;
     $HOME/.pyenv/versions/3.5.2/bin/python3.5 ./mk_wheel.py -T -j 1 -DCUDA_ROOT=/usr/local/cuda;
fi
...
if [ "${CB_BUILD_AGENT}" == 'python36-darwin-x86_64-release' ]; then
     ...
     brew install pyenv;
     pyenv install 3.6.3;
     $HOME/.pyenv/versions/3.6.3/bin/python3.6 ./mk_wheel.py -T -j 1 -DCUDA_ROOT=/usr/local/cuda;
fi
